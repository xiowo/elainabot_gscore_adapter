...
以下指令默认使用 `#早柚` 作为前缀；如果在 WebUI 中修改了 **群指令前缀**，请将示例中的 `#早柚` 替换为你设置的新前缀。

| 指令 | 说明 |
| :--- | :--- |
| `#早柚help` | 显示适配器帮助菜单 |
| `#早柚status` | 查看连接状态、启用 Bot 数量、黑名单数量等信息 |
| `#早柚version` | 查看适配器版本 |
| `#早柚重连` | 手动重连 GScore，最多尝试 3 次；自动重连中会提示稍后查看状态 |
| `#早柚群禁用` | 关闭本群消息转发至 GScore |
| `#早柚群启用` | 开启本群消息转发至 GScore |
| `#早柚拉黑 @用户` | 将指定用户加入黑名单，不再转发其消息 |
| `#早柚取消拉黑 @用户` | 将指定用户移出黑名单 |

> 管理指令仅机器人主人可用；开启 **无权限静默** 后，非主人使用管理指令时不会返回权限提示。
